...
def _build_loops_from_boundary_edges(edges):
    ...
    for u, v in edges:
        ...
        while True:
            loop.append(cur)
            nbs = adj.get(cur, [])
            cand = [nb for nb in nbs if nb != prev]

            if len(cand) == 0:
                loop = None
                break

            if start in cand and len(loop) >= 3:
                cur = start
                break

            unvisited = [nb for nb in cand if edge_key(cur, nb) not in visited]
            if len(unvisited) == 0:
                loop = None
                break
        ...
        if loop is not None and cur == start and len(loop) >= 3:
            loops.append(loop)
...
def suggest_alpha_from_knn(xy: np.ndarray, k: int = 10, factor: float = 2.5, n_probe: int = 5000, seed: int = 0) -> float:
    ...
    tree = cKDTree(pts)
    dists, _ = tree.query(pts, k=min(k + 1, pts.shape[0]))  # includes self 0 
    dk = np.median(dists[:, -1])  # the k neighbor
