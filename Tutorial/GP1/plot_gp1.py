...
def _set_ax_common(ax, xlim, ylim, invert_y=False):
    ax.set_xlim(*xlim)
    ax.set_ylim(*ylim)
    ax.set_aspect("equal")
    if invert_y:
...
def plot_oneframe(
    res, adata_all, *,
    route_ids,                 # [head, tail]
    real_mid_id=None,          # e.g. "U22"; if None, try infer midpoint
    ...
    for ax in (ax0, ax1, ax2):
        _set_ax_common(ax, xlim, ylim, invert_y=invert_y)
...
def plot_allframes(
    ...
    for col in range(n_cols):
        ...
        x0 = trace["x"][0]
        N0 = int(x0.shape[0]) if torch.is_tensor(x0) else int(np.asarray(x0).shape[0])
...
# ============================================================
# (C) all policy rollout frames
# vertical: rows=time, cols=segments
# horizontal: rows=segments, cols=time
# ============================================================
def plot_allframes_policy(
    rollouts, adata_all, ctx, *,
    route_ids,                 # chain like ['U2','U12','U22','U32']
    steps=None,                # if None -> infer max Tp1 across segments
    sample_key="sample",
    ann_key="His_anno",
    x_key="cx_aligned", y_key="cy_aligned",
    n_show=20000, s=3, alpha=0.85, seed=0,
    show_birth=False,
    birth_scale=6.0,
    birth_edgecolor="r",
    birth_lw=0.6,
    ...
    for seg_i in range(n_seg):
        ...
        # ----- limits per segment -----
        mins, maxs = [], []
        idx_s = _pick_local(xy_s.shape[0], n_show, seed + 2000 + seg_i)
        xs = xy_s[idx_s]
        mins.append(xs.min(0)); maxs.append(xs.max(0))
        ...
        for i in range(n_frames):
            ...
            if xi.shape[0] == 0:
                continue
            idx_i = _pick_local(xi.shape[0], n_show, seed + 10000 + seg_i*100 + i)
            xii = xi[idx_i]
            mins.append(xii.min(0)); maxs.append(xii.max(0))

        idx_e = _pick_local(xy_e.shape[0], n_show, seed + 3000 + seg_i)
        xe = xy_e[idx_e]
        mins.append(xe.min(0)); maxs.append(xe.max(0))
        ...
        # ----- draw start real -----
        ax = _ax(0, col) if layout == "vertical" else _ax(row, 0)
        cs = col_s[idx_s]
        ax.scatter(xs[:, 0], xs[:, 1], c=cs, s=s, alpha=alpha, linewidths=0)
        ...
        for i in range(n_frames):
            ...
            ax = _ax(1 + i, col) if layout == "vertical" else _ax(row, 1 + i)
        ...
        # ----- draw end real -----
        ax = _ax(n_rows - 1, col) if layout == "vertical" else _ax(row, n_cols - 1)
        ce = col_e[idx_e]
        ax.scatter(xe[:, 0], xe[:, 1], c=ce, s=s, alpha=alpha, linewidths=0)
